...
func (fxd *FxDaemon) StartAPIServer(mux *http.ServeMux) {
	...
	transfer_container := func(w http.ResponseWriter, r *http.Request){
		...
		for _, img := range imgs {
			fmt.Println(img.RepoTags[0], image_name)
			if img.RepoTags[0] == image_name {
				cont, cont_error := client.CreateContainer(docker.CreateContainerOptions{
					...
					Config: &docker.Config{Cmd: []string{command}, Image: img.RepoTags[0],AttachStdin: false, AttachStderr: false, AttachStdout: false},
